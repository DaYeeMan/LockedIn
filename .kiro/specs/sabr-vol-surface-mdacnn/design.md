...
- `funahashi_baseline.py`: Exact implementation of Funahashi's 5-layer residual ANN
- `baseline_models.py`: Additional MLP baselines for comparison
...
**Key Components**:
- `metrics.py`: Custom evaluation metrics including Funahashi's MSE and relative percentage error
- `surface_evaluator.py`: Surface-specific evaluation logic
- `funahashi_comparison.py`: Direct comparison with Funahashi's published results
- `benchmark_comparison.py`: Compare against all baseline models
- `statistical_analysis.py`: Statistical significance testing for research publication

**Evaluation Metrics**:
- RMSE/MAE for overall surface accuracy
- Relative error percentages
- ATM/ITM/OTM region-specific metrics
- Deep wing performance analysis (where MC-Hagan residuals are largest)
- Wing error reduction compared to Funahashi baseline
...
**Key Components**:
- `data_visualizer.py`: MC and Hagan surface visualization during data generation
- `surface_plotter.py`: 3D surface visualization for model results
- `smile_plotter.py`: Volatility smile comparisons
- `residual_plotter.py`: Residual analysis and wing region identification
...
**Key Visualizations**:
- Raw data visualization: MC vs Hagan surface comparisons during data generation
- Residual heatmaps showing spatial patterns in MC-Hagan differences
- Wing region identification plots highlighting areas of largest residuals
...
class SABRParams:
    ...
    alpha: float   # Initial volatility [0.05-0.6] (Funahashi range)
    beta: float    # Elasticity parameter [0.3-0.9] (Funahashi range)
    nu: float      # Vol-of-vol parameter [0.05-0.9] (Funahashi range)
    rho: float     # Correlation parameter [-0.75-0.75] (Funahashi range)
